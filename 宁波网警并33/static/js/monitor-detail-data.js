...
function load(){	
	//获取数据
	http.post('/mobile/desktop/showBillWorking.html',{websiteId:billId}).done(function(data) {
		console.log(data);
	app1.fzrid=data.website.fzrid;
	...
	defualtDay=today;
	app1.cfName=data.tlList[0].value;
	app1.cfid=data.tlList[0].key;
	app1.vul=data.data.vul.isWorkingCount;
	app1.leak=data.data.link.isWorkingCount;
	app1.dns=data.data.dns.isWorkingCount;
	app1.all=app1.dns+app1.leak+app1.vul;
	//通报内容
	var result = '';
	var arrLen = data.tlList.length;
	for(var i = 0; i < arrLen; i++) {
		result += '<li data-id="'+data.tlList[i].key+'" id="a'+i+'">' + data.tlList[i].value + '</li>';
	}
