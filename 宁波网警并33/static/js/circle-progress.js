...
(function () {
    window.CircleProgress = function (el, startColor, endColor) {
        ...
        function drawOutArc(endDeg, isLast) {
            ...
            ctx.beginPath();
            ctx.fillStyle='#fff';
            var fontSize = el.style.fontSize || '20px';
            ctx.font= fontSize + " Microsoft yahei";
            var result = Math.round(currentShowValue) + '%';
            var resultTextObject = ctx.measureText(result);
            ctx.fillText(result,  circle.x - resultTextObject.width / 2, circle.y + fontSize.replace('px', '') / 2 - 6);
            ctx.stroke();
