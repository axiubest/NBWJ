	 ...
	 series: { 
	 	 cursor: 'pointer', 
            events: { 
                click: function(e) { 
                	var i=e.point.x;//获取点击x坐标序列0开始
                	if(chart5y1[i]==undefined){chart5y1[i]=0}
                	if(chart5y2[i]==undefined){chart5y2[i]=0}
                	if(chart5y3[i]==undefined){chart5y3[i]=0}
                	$('#overdue5').text(chart5y3[i]);//过期单
                	$('#sheet5').text(chart5y1[i]);//通报单
                	$('#rate5').text(chart5y2[i]+'%');//完成率
                } 
            } ,
		 marker:{ 
		        radius:4,  //曲线点半径，默认是4圆点大小
